...
(defn update-service-version [service]
  (when service
    (assoc (service-version-map service)
           (keyword (service-protocol/version service)) service)))

(defn add-service [service]
  (swap! services assoc (keyword (service-protocol/key service))
         (update-service-version service)))
...
(defn service-not-found [service-key service-version]
  (logging/error (str "Service not found: " service-key ", version: "
                      service-version))
